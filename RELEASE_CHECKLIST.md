...
- [ ] Test a free recursive capture that exceeds the remaining allowance, confirm the complete interaction is saved, and confirm the next save is blocked.
...
- Static article, image-heavy article, CSS from a CDN, fonts from another domain, single-page app, authenticated page, redirect, offline open, failed asset, large direct media, depth 1–5, free-user 250-page/1 GiB caps, Pro 500/1,000-page and 2/4 GiB settings, delete/reorder/search, browser restart mid-capture, and month rollover.
- Windows, macOS, ChromeOS if available; normal and offline network conditions; light and dark source pages; keyboard-only popup use; 200% zoom; and reduced motion.
